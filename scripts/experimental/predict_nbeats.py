"""
Simple script to make predictions using a trained N-BEATS model.

Usage:
    python predict_nbeats.py --checkpoint checkpoints/nbeats-epoch=15-val_loss=0.45.ckpt
"""
...
from train_nbeats import load_and_prepare_data, create_datasets, create_model
...
def make_predictions(checkpoint_path, training_dataset, test_dataset, n_samples=5, save_plots=True):
    ...
    nbeats_model.load_state_dict(checkpoint["state_dict"])
    nbeats_model.eval() # Set to evaluation mode
    ...
    # N-BEATS predict method usually returns a single tensor for point predictions
    predictions = nbeats_model.predict(
        test_dataloader,
        mode="prediction", # N-BEATS typically does not have "raw" or "quantiles" modes by default
        return_x=True,
        return_y=True,
    )

    # Extract results
    pred_output_point = predictions.output  # This will be a single tensor of point predictions
    pred_x = predictions.x  # x_data used for predictions
    pred_y = predictions.y  # Actual values

    print(f"\nPrediction shape (point forecasts): {pred_output_point.shape}")
    print(f"  - {pred_output_point.shape[0]} samples")
    print(f"  - {pred_output_point.shape[1]} timesteps ahead")


    if save_plots:
        # N-BEATS does not have variable importance plotting like TFT
        ...
        for i in range(min(n_samples, pred_output_point.shape[0])): # Use shape[0] for actual samples
            idx = np.random.randint(0, pred_output_point.shape[0]) # Random index within the batch
            ...
            predicted = pred_output_point[idx, :].cpu().numpy() # Point prediction
            ...
            # Predicted future
            ax.plot(pred_x_axis, predicted, 'r--', label='Predicted', linewidth=2)
            ...
            plt.tight_layout()
            output_file = f'nbeats_prediction_sample_{i+1}.png'
            plt.savefig(output_file, dpi=150, bbox_inches='tight')
            print(f"  ✓ Saved: {output_file}")
            plt.close()

    # Calculate metrics
    preds_flat = pred_output_point.cpu().numpy().flatten()
    ...
    horizon_idx = np.tile(np.arange(pred_output_point.shape[1]), pred_output_point.shape[0])
    ...
    # N-BEATS does not provide quantiles directly, so PICE is not applicable
    print("  PICE: Not applicable for N-BEATS point predictions.")
...
def predict_future(checkpoint_path, n_days=7):
    ...
    # Load model
    # N-BEATS does not have load_from_checkpoint. We need to re-initialize and load state_dict.
    # For now, this is a placeholder. Future prediction is complex and usually involves iterative forecasting.
    print("Note: Full future prediction is not yet implemented for N-BEATS.")
    return None

def main():
    parser = argparse.ArgumentParser(description='Make predictions with trained N-BEATS')
